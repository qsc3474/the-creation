...
public class AjaxCommentSelectListServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 부모댓글들로 묶어서 넘기기 위해 인스턴스 생성
		List<CommentDTO> parentCommentList = new ArrayList<>();
		
		// 부모댓글번호가 없는(자신이 부모 혹은 독립적인 댓글)댓글일시 부모댓글리스트로 묶는다
		...
		for(int i = 0 ; i < commentList.size() ; i++) {
			...
			// 셀렉해서 가져온 모든 댓글에 대해서 부모댓글이 존재하는 댓글일시 진입
			if(comment.getParentCmtNo() != 0) {
				
				// 부모댓글의 번호를 추출
				int parentNo = comment.getParentCmtNo();
		...
		String jsonString = new Gson().toJson(parentCommentList);
